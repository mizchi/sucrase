...
function assertScopes(code: string, expectedScopes: Array<Scope>): void {
  assert.deepEqual(parse(code, false).scopes, expectedScopes);
}

test("properly provides function and program scopes", () => {
  assertScopes(
    `
      const x = 1;
      function foo(x) {
...
test("handles scopes for loops, conditionals, etc", () => {
  assertScopes(
    `
      const x = 1;
      function foo(x) {
...
test("handles functions in different contexts", () => {
  assertScopes(
    `
...
test("creates a special name scope for function expressions but not statements", () => {
...
test("creates a special name scope for class expressions but not statements", () => {
