 ...
 */
test("handles keywords as object keys", () => {
  assertResult(
    `
...
test("allows keywords as object keys", () => {
  assertResult(
    `
      const o = {
        function: 3,
...
test("allows computed class method names", () => {
  assertResult(
    `
      class A {
        [b]() {
...
test("supports getters and setters within classes", () => {
  assertResult(
    `
      class A {
        get foo(): number {
...
test("handles properties named `case`", () => {
  assertResult(
    `
      if (foo.case === 3) {
      }
...
test("handles labeled switch statements", () => {
  assertResult(
    `
      function foo() {
        outer: switch (a) {
...
test("handles methods called `get` and `set` in a class", () => {
  assertResult(
    `
      class A {
        get() {
...
test("handles generator class methods", () => {
  assertResult(
    `
      class A {
        *foo() {
...
test("handles async generator class methods", () => {
  assertResult(
    `
      class C {
        async *m() {
...
test("removes numeric separators from number literals", () => {
  assertResult(
    `
      const n = 1_000_000;
      const x = 12_34.56_78;
...
// test.skip("allows using the import keyword as an export", () => {
//   assertResult(
//     `
//       const Import = null;
...
test("properly converts static fields in statement classes", () => {
  assertResult(
    `
      class A {
        static x = 3;
...
test("properly converts static fields in expression classes", () => {
  assertResult(
    `
      const A = class {
        static x = 3;
...
test("properly converts instance fields in expression classes", () => {
  assertResult(
    `
      const A = class {
        x = 3;
...
test("properly converts exported classes with static fields", () => {
  assertResult(
    `
...
test("properly resolves imported names in class fields", () => {
  assertResult(
    `
      import A from 'A';
      import B from 'B';
...
test("puts the prefix after a shebang if necessary", () => {
  assertResult(
    `#!/usr/bin/env node
      console.log("Hello");
    `,
...
test("handles optional catch binding", () => {
  assertResult(
    `
      const e = 3;
      try {
...
test("handles prefix operators with a parenthesized operand", () => {
  assertResult(
    `
      const x = +(y);
    `,
...
test("handles async object methods", () => {
  assertResult(
    `
      const o = {
        async f() {
...
// test.skip("handles decorated classes with static fields", () => {
//   assertResult(
//     `
...
test("handles logical assignment operators", () => {
  assertResult(
    `
      a &&= b;
      c ||= d;
...
test("handles decorators with a parenthesized expression", () => {
  assertResult(
    `
      class Bar{
        @(
...
test("handles variables named createReactClass", () => {
  assertResult(
    `
...
test("handles a static class field without a semicolon", () => {
  assertResult(
    `
      class A {
        static b = {}
...
test("handles a class with class field bound methods", () => {
  assertResult(
    `
...
test("removes semicolons from class bodies", () => {
  assertResult(
    `
      class A {
        ;
...
test("removes comments within removed ranges rather than removing all whitespace", () => {
  assertResult(
    `
      interface A {
        // This is a comment.
...
test("handles static class fields with non-identifier names", () => {
  assertResult(
    `
      class C {
        static [f] = 3;
...
test("preserves line numbers for multiline fields", () => {
  assertResult(
    `
      class C {
        f() {
...
test("allows a class expression followed by a division operator", () => {
  assertResult(
    `
      x = class {} / foo
    `,
...
test("handles newline after async in paren-less arrow function", () => {
  assertResult(
    `
...
test("handles various parser edge cases around regexes", () => {
  assertResult(
    `
      for (const {a} of /b/) {}
...
test("handles an arrow function with trailing comma params", () => {
  assertResult(
    `
      const f = (
        x: number,
...
test("handles a file with only a single identifier", () => {
  assertResult("a", "a", {transforms: []});
});

test("handles a file with only an assignment", () => {
  assertResult("a = 1", "a = 1", {transforms: []});
});

test("handles a standalone comment that looks like it could be a regex", () => {
  assertResult(
    `
      /*/*/;
    `,
...
test("handles a comment that looks like it could be a regex after a string", () => {
  assertResult(
    `
      let thing = "sup" /*/*/;
    `,
...
test("handles smart pipeline syntax", () => {
  assertResult(
    `
      value |> #
      value |> (#)
...
test("handles partial application syntax", () => {
  assertResult(
    `
      foo(?)
    `,
...
test("handles V8 intrinsic syntax", () => {
  assertResult(
    `
      %DebugPrint(foo)
    `,
...
test("handles comments after trailing comma after elision", () => {
  assertResult(
    `
...
test("transpiles basic nullish coalescing", () => {
  assertResult(
    `
      const x = a ?? b;
    `,
...
test("transpiles nested nullish coalescing", () => {
  assertResult(
    `
      const x = a ?? b ?? c;
    `,
...
test("handles falsy LHS values correctly in nullish coalescing", () => {
  assertOutput(
    `
...
test("handles nested nullish coalescing operations", () => {
  assertOutput(
    `
...
test("transpiles various optional chaining examples", () => {
  assertResult(
    `
...
test("correctly accesses a nested object when using optional chaining", () => {
  assertOutput(
    `
      const a = {b: {c: 15}}
      setOutput(a?.b.c ?? 10);
...
test("correctly falls back to a default when using optional chaining and nullish coalescing", () => {
  assertOutput(
    `
      const a = null;
      setOutput(a?.b.c ?? 10);
...
test("correctly transpiles nullish coalescing with an object right-hand side", () => {
  assertResult(
    `
      null ?? {x: 5}
    `,
...
test("correctly handles nullish coalescing with an object on the right-hand side", () => {
  assertOutput(
    `
...
test("specifies the proper this when using optional chaining on a function call", () => {
  assertOutput(
    `
      class A {
        constructor(value) {
...
test("transpiles optional chain deletion", () => {
  assertResult(
    `
      delete a?.b.c;
    `,
...
test("correctly identifies last element of optional chain deletion", () => {
  assertResult(
    `
      delete a?.b[c?.c];
    `,
...
test("deletes the property correctly with optional chain deletion", () => {
  assertOutput(
    `
      const o = {x: 1};
      delete o?.x;
...
test("does not crash with optional chain deletion on null", () => {
  assertOutput(
    `
      const o = null;
      delete o?.x;
...
test("does not crash with nullish coalescing followed by TS `as`", () => {
  assertOutput(
    `
...
test("correctly transforms nullish coalescing followed by TS `as`", () => {
  assertResult(
    `
      const foo = {
        bar: baz ?? null as any
...
test("correctly transforms async functions using await in an optional chain", () => {
  assertResult(
    `
      async function foo() {
        a?.b(await f())
...
test("does not mistake an unrelated await keyword for an async optional chain", () => {
  assertResult(
    `
      function foo() {
        a?.b(async () => await f())
...
test("correctly transforms async functions using await in an optional chain deletion", () => {
  assertResult(
    `
      async function foo() {
        delete a?.[await f()];
...
test("correctly transforms async functions using await in nullish coalescing", () => {
  assertResult(
    `
      async function foo() {
        return a ?? await b();
...
test("allows super in an optional chain", () => {
  assertResult(
    `
      class A extends B {
        foo() {
...
test("allows a super method call in an optional chain", () => {
  assertResult(
    `
      class A extends B {
        foo() {
...
test("allows bigint literals as object keys", () => {
  assertResult(
    `
      const o = {0n: 0};
    `,
...
test("allows decimal literals as object keys", () => {
  assertResult(
    `
      const o = {0m: 0};
    `,
...
test("parses and passes through class static blocks", () => {
  assertResult(
    `
      class A {
        static {
...
test("correctly handles scope analysis within static blocks", () => {
  assertResult(
    `
...
test("correctly preserves private fields, static fields, and methods", () => {
  assertResult(
    `
      class A {
        #x = 1;
